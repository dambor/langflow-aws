...
```bash
aws iam get-role --role-name LangflowS3Role --query 'Role.AssumeRolePolicyDocument'
```

### Issue: Access denied to S3

**Check:**
1. Role has S3 permissions attached
2. Bucket policy doesn't explicitly deny access
3. The S3 key/prefix exists in the bucket
4. Bucket is in the specified region

**Verify permissions:**
```bash
aws iam list-attached-role-policies --role-name LangflowS3Role
```

**Test access directly:**
```bash
aws s3api head-object --bucket YOUR_BUCKET_NAME --key YOUR_S3_KEY
```

### Issue: "boto3 is not installed"

Install boto3 in your Langflow environment:

```bash
pip install boto3
```

Or add it to your container image's requirements.

### Issue: Downloads directory permission denied

Ensure the Langflow pod has write access to the local download path. If using a volume mount, check permissions:

```bash
kubectl exec -n langflow $POD_NAME -- ls -la /app/downloads
```
...
For production use, consider using a PersistentVolumeClaim instead of `emptyDir`:

```yaml
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: langflow-downloads
  namespace: langflow
spec:
  accessModes:
    - ReadWriteOnce
...
1. **Least Privilege**: Grant only `s3:GetObject`, `s3:HeadObject`, and `s3:ListBucket` — never `s3:*`
2. **Bucket Scoping**: Restrict IAM policies to specific bucket ARNs, not `*`
3. **Audit Logging**: Enable CloudTrail and S3 access logging to audit download activity
4. **Encryption**: Ensure S3 buckets use server-side encryption (SSE-S3 or SSE-KMS)
5. **Network Policies**: Use Kubernetes network policies to restrict pod communication
6. **Regular Review**: Periodically audit role policies and service account bindings
7. **No Hardcoded Credentials**: Prefer IRSA or role assumption over direct access keys
...
✅ **Granular permissions** — Different pods can have different S3 access  
✅ **Audit trail** — CloudTrail shows which pod accessed which S3 objects  
...
- [S3 IAM Policy Examples](https://docs.aws.amazon.com/AmazonS3/latest/userguide/example-policies-s3.html)
...
For local development (laptop), you can still use the role assumption approach from the component, or simply use AWS CLI credentials:

```bash
# Configure AWS CLI
aws configure

# Leave all credential fields empty in the component
